...
export async function POST(request: NextRequest) {
  try {
    ...
    // 使用情感领域的配置作为默认配置
    const defaultConfig: Record<string, ArticleStyle> = articleStyles;
    const articleStyle: ArticleStyle = defaultConfig['情感'];
    const { style, rewrite_instruction, prompt: basePrompt } = articleStyle;
