...
export async function POST(request: NextRequest) {
  try {
    const { content, theme, field = '全部' } = await request.json();
    if (!content || !theme) {
    ...
    // 使用指定领域或默认配置
    const articleStyle: ArticleStyle = articleStyles[field] || defaultConfig['全部'];
    const { style, humanize_tips, rewrite_instruction } = articleStyle;
